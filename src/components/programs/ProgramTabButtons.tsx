...
export default function ProgramTabButtons({
  programData,
  title,
}: {
  programData: Program[];
  ...
  return (
    <div>
      <h3 className="mb-[22px] text-[34px] font-bold text-black">{title}</h3>
      <div className="rounded-sm border border-stroke">
        {programData.map((program: Program) => (
          <Link
            href={
              program.is_active === false ? "#" : `/programs/${program?.id}`
            }
            key={program?.id}
            className={cn(
              `flex w-full items-center gap-3 border-b border-stroke px-5 py-4 text-lg font-medium duration-200 last-of-type:border-0 ${pathname === `/programs/${program?.id}` ? "text-primary" : "text-black hover:text-primary"}`,
              {
